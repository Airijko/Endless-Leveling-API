...
public final class EndlessLevelingAPI {
    ...
    private final Set<String> runtimeMobBlacklistView = Collections.unmodifiableSet(runtimeMobBlacklist);
    ...
    public Set<SkillAttributeType> getHiddenSkillAttributes() {
        EnumSet<SkillAttributeType> snapshot = EnumSet.noneOf(SkillAttributeType.class);
        snapshot.addAll(hiddenSkillAttributes);
        return snapshot.isEmpty() ? Set.of() : Collections.unmodifiableSet(snapshot);
    }
    ...
    public void addMobBlacklistEntry(String entry) {
        if (entry != null && !entry.isBlank()) {
            runtimeMobBlacklist.add(normalizeMobEntry(entry));
        }
    }
    ...
    public void removeMobBlacklistEntry(String entry) {
        if (entry != null && !entry.isBlank()) {
            runtimeMobBlacklist.remove(normalizeMobEntry(entry));
        }
    }
    ...
    public boolean hasMobBlacklistEntry(String entry) {
        ...
        return runtimeMobBlacklist.contains(normalizeMobEntry(entry));
    }
    ...
    public Set<String> getRuntimeMobBlacklistView() {
        return runtimeMobBlacklistView;
    }
    ...
    public boolean isAutoAllocateAllowed(UUID playerUuid) {
        ...
        for (Predicate<UUID> guard : autoAllocateGuards) {
            try {
                ...
                // Treat a broken guard as a block rather than crashing level-up processing.
                return false;
            }
        }
        return true;
    ...
    /**
     * Mirrors MobLevelingManager.normalizeMobType() so runtime blacklist entries
     * are compared on the same footing as config-file rules.
     */
    private static String normalizeMobEntry(String raw) {
        return raw.trim()
                .replace('-', '_')
                .replace('.', '_')
                .replace(' ', '_')
