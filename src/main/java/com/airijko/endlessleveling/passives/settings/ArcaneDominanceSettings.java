...
/**
 * Encapsulates Arcane Dominance tuning (health scaling + on-hit slow).
 */
public record ArcaneDominanceSettings(boolean enabled,
        double sorceryFromTotalHealthPercent,
        double targetHasteSlowOnHitPercent,
        double targetHasteSlowDurationSeconds) {

    private static final double DEFAULT_SORCERY_FROM_TOTAL_HEALTH_PERCENT = 0.05D;
    ...
    public static ArcaneDominanceSettings fromSnapshot(ArchetypePassiveSnapshot snapshot) {
        ...
        double sorceryFromTotalHealthPercent = Math.max(0.0D, snapshot.getValue(ArchetypePassiveType.ARCANE_DOMINANCE));
        ...
        for (RacePassiveDefinition definition : definitions) {
            ...
            double scalingCandidate = parsePercent(props,
                    "sorcery_from_total_health_percent",
                    "total_health_scaling_percent",
                    "health_scaling_percent",
                    "from_total_health_percent");
            if (scalingCandidate > 0.0D) {
                sorceryFromTotalHealthPercent = Math.max(sorceryFromTotalHealthPercent, scalingCandidate);
            }
        ...
        if (sorceryFromTotalHealthPercent <= 0.0D) {
            sorceryFromTotalHealthPercent = DEFAULT_SORCERY_FROM_TOTAL_HEALTH_PERCENT;
        }

        return new ArcaneDominanceSettings(true,
                sorceryFromTotalHealthPercent,
                targetSlowPercent,
                targetSlowDuration);
    }
    ...
    public static ArcaneDominanceSettings disabled() {
        return new ArcaneDominanceSettings(false, 0.0D, 0.0D, 0.0D);
    }
